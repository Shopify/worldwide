...
describe('getConcatenationRules', () => {
  describe('with address object', () => {
    ...
    test('returns default rules if script specific rules are defined but script is not detected', () => {
      const config = getRegionConfig('TW');
      const address: Address = {
        countryCode: 'TW',
        line2: '30號',
    ...
    test('returns default rules if script specific rules but multiple scripts detected', () => {
      const config = getRegionConfig('TW');
      const address: Address = {
        countryCode: 'TW',
        line2: 'No. 30號',
...
describe('addressUsesScript', () => {
  test.only('returns true for Latin in US English examples', () => {
    ...
    expect(addressUsesScript(fieldDefinition, fieldValues, 'Latin')).toBe(true);
  });
  ...
  test('returns false for Latin in Taiwan Chinese examples', () => {
    ...
    expect(addressUsesScript(fieldDefinition, fieldValues, 'Latin')).toBe(
      false,
    );
  });
